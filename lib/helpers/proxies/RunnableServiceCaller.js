...
module.exports.getOperationCall = function getOperationCall(service, operation) {
    return function _operationCall(expressRequest, expressResponse) {
        ...
        service[operation](operationRequest, operationResponse, () => {
...
module.exports.getCheckOperationRequestCall = function getCheckOperationRequestCall(service, operation) {
    return function _checkOperationRequestCall(expressRequest, expressResponse, next) {
        ...
        if (checkTypes.not.object(operationRequestInfo) || checkTypes.emptyObject(operationRequestInfo)) {
        ...
        if (checkTypes.not.object(parametersOperation) || checkTypes.emptyObject(parametersOperation)) {
            next();
            return;
        }

        for (const paramName of Object.keys(parametersOperation)) {
            ...
            if (paramRequired === true) {
                ...
                if (checkTypes.not.assigned(paramValue) || checkTypes.emptyString(paramValue)) {
                    expressResponse.status(400);
