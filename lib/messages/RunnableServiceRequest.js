 ...
 * @class
 */
module.exports.RunnableServiceRequest = class RunnableServiceRequest {
    /**
    ...
    getParameters() {
        ...
        const pathParametersNames = Object.keys(this.pathParameters);
        for (let i = 0; i < pathParametersNames.length; i++) {
            const pathParameter = pathParametersNames[i];
            parameters[pathParameter] = this.pathParameters[pathParameter];
        }

        const queryParametersNames = Object.keys(this.queryParameters);
        for (let j = 0; j < queryParametersNames.length; j++) {
            const queryParameter = queryParametersNames[j];
