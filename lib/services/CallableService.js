...
module.exports = class CallableService extends Service {
     ...
     * @returns {Promise} - Promise that resolves with the response received from the remote service and rejects if an error occurs.
     */
    execute(request) {
...
function _executeOperation(service, request) {
    return new Promise((resolve, reject) => {
        ...
        if (checkTypes.object(realParameters) && checkTypes.not.emptyObject(realParameters)) {
            for (const paramName of Object.keys(realParameters)) {
